<div align="center">

```
███╗   ███╗██╗███████╗██╗  ██╗ █████╗ 
████╗ ████║██║██╔════╝██║  ██║██╔══██╗
██╔████╔██║██║███████╗███████║███████║
██║╚██╔╝██║██║╚════██║██╔══██║██╔══██║
██║ ╚═╝ ██║██║███████║██║  ██║██║  ██║
╚═╝     ╚═╝╚═╝╚══════╝╚═╝  ╚═╝╚═╝  ╚═╝
```

<img src="https://readme-typing-svg.demolab.com/?font=Fira+Code&size=22&pause=1000&color=39D353&center=true&vCenter=true&width=600&lines=Building+CrisisSync+%E2%80%94+offline-first+crisis+coordination;Shipping+ML+across+web%2C+edge+%26+embedded;SIH+2026+%E2%80%94+industrial+fire+detection+w%2F+NASA+FIRMS" alt="typing banner" />

I build things that need to work when infrastructure doesn't — offline-first apps, edge ML on real hardware, and satellite data pipelines.
Currently: **CrisisSync** and an SIH 2026 industrial-fire detection system.

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=whit)](https://www.linkedin.com/in/shaik-misha-tabassum-988140275/)
[![Email](https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:shaikmisha2005@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/ShaikMishaTabassum)

</div>

---

## 🛠 Languages & Tools

![stack](https://skillicons.dev/icons?i=fastapi,postgres,react,nextjs,py,pytorch,tensorflow,flutter,cpp,docker,go,git,linux,aws,figma)

---

## 🚀 Selected work

| Project | What it does | Stack |
|---|---|---|
| **[CrisisSync](https://github.com/ShaikMishaTabassum/crisissync)** | Offline-first crisis coordination platform, real-time via WebSockets | FastAPI, PostgreSQL/Supabase, Next.js, React Three Fiber |
| **[RailGuard AI](https://github.com/ShaikMishaTabassum/railguard-ai)** | Edge safety monitoring on real hardware, multi-agent AI council | ESP32, TinyML/Edge Impulse, FastAPI, Groq/Llama-3.3 |
| **[Shipwright](https://github.com/ShaikMishaTabassum/shipwright)** | CI/CD + Kubernetes deploy tool, CLI + dashboard | Go, k3s, Terraform, GitHub Actions |
| **[SHEild](https://github.com/ShaikMishaTabassum/sheild)** | Women's-safety wearable, on-device inference | Flutter, ESP32, TensorFlow Lite |
| **[CreditMirror](https://github.com/ShaikMishaTabassum/creditmirror)** | Alternative credit scoring for credit-invisible Indians | XGBoost/SHAP, FastAPI, React |
| **[Kagaz](https://github.com/ShaikMishaTabassum/kagaz)** | Voice-first multilingual govt document navigator | FastAPI, PostgreSQL, Gemini API |

---



## 🕹 Contribution graph

![pacman contribution graph](https://raw.githubusercontent.com/ShaikMishaTabassum/ShaikMishaTabassum/output/pacman-contribution-graph.svg)
